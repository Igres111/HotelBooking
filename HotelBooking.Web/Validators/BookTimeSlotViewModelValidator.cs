...
{
    public class BookTimeSlotViewModelValidator : AbstractValidator<BookTimeSlotViewModel>
    {
        public BookTimeSlotViewModelValidator()
        {
            ...
            RuleFor(model => model)
                .Must(model => BookingValidationHelper.IsInTheFuture(model.Date, model.StartTime, model.TimeZoneId))
                .WithName("StartTime")
                .WithMessage("Start time must be in the future.")
                .When(model => BookingValidationHelper.IsValidTimeZone(model.TimeZoneId));

            RuleFor(model => model)
                .Must(model => BookingValidationHelper.IsWithinAdvanceBookingWindow(model.Date, model.TimeZoneId))
                .WithName("Date")
