...
namespace HotelBooking.Validators
{
    public class CreateBookingRequestValidator : AbstractValidator<CreateBookingRequest>
    {
        public CreateBookingRequestValidator(ITimeZoneConverter timeZoneConverter)
        {
            ...
            RuleFor(request => request)
                .Must(BookingValidationHelper.IsDurationWithinAllowedRange)
                .WithName("EndTime")
                .WithMessage("Duration must be between 30 minutes and 4 hours.")
                .When(request => request.EndTime > request.StartTime);

            RuleFor(request => request)
                .Must(BookingValidationHelper.IsThirtyMinuteIncrement)
                .WithName("EndTime")
                .WithMessage("Duration must be in 30-minute increments.")
                .When(request => request.EndTime > request.StartTime);

            RuleFor(request => request)
                .Must(request => BookingValidationHelper.IsInTheFuture(request, timeZoneConverter))
                .WithName("StartTime")
                .WithMessage("Start time must be in the future.")
                .When(request => BookingValidationHelper.IsValidTimeZone(request.TimeZoneId));

            RuleFor(request => request)
                .Must(request => BookingValidationHelper.IsWithinAdvanceBookingWindow(request, timeZoneConverter))
                .WithName("Date")
